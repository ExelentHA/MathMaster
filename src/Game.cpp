...
void Game::Init()
{
    ...
    btn_question3.Init("res/ttf/Playtime.ttf", (3*WIDTH/4)-15, (HEIGHT/2)+60, 0.4f, Window->renderer, true );
    btn_question4.Init("res/ttf/Playtime.ttf", (3*WIDTH/4)-15, (21*HEIGHT)/24, 0.4f, Window->renderer, true);
    
    question.Init("res/ttf/Playtime.ttf", ((2*WIDTH)/4), (HEIGHT/4)+ -30, 0.3f, Window->renderer, false);

    tb_menu1.Init("res/ttf/Playtime.ttf", ((2*WIDTH)/4), (2*HEIGHT/5)-12, 0.2f, Window->renderer, false);
    tb_menu1.SetRenderSpeed(0.5);
    tb_menu2.Init("res/ttf/Playtime.ttf", ((2*WIDTH)/4), (HEIGHT/8), 0.2f, Window->renderer, false);
    tb_menu1.SetRenderSpeed(0.5);
...
void Game::InitMenu()
{
    if(!isAlreadyMenu) // initialize texts
    {
        ...
        clock.SetTime(0);
        isAlreadyMenu = true; // so these function will not initialize again   
    }
}
...
void Game::GenQAdd()
{
    ...
    if(result != 0) // in certain senario the program will generate a 0 result leading for wierd set of answer choices
    {
        if (result <= 4) // fewer choice of selection will result into never ending loop
        {                   // the program will not generate multiple answer duplicates
            min = result - 10; // setting the range based on the answer
            max = result + 10;
        }
        else
        { // Modifiers in level
